...
if(!$result) 
{ 
    header("Location:change.php?err=<h3>You entered an incorrect password.</h3>");
} 
...
if(mysql_num_rows($result)){
    if($newpassword == $confirmpassword)
	{
        ...
        if($sql) 
        { 
            header("Location:main.php?err=<h3>Congratulations! You have successfully changed your password.</h3>"); 
        }
        else
        {
            // In case when problem while updating your new password
			header("Location:change.php?err=<h3>Not updated your new password.</h3>");
        }       
    } else {
        // In case when new-password and retype-password do not match
		header("Location:change.php?err=<h3>The new password and confirm new password fields must be the same.</h3>");
    }
} else {
    // In case of you have not correct User name and password
	header("Location:change.php?err=<h3>Your username or password incorrect!</h3>");
}
